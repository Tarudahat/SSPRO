...
async function apply_theme() {
    ...
    if (cstm_theme_used) {
        ...
        await browser.storage.local.get("bg2_color").then((a) => {
            let e = document.createElement("style");
            e.innerText = ".treecollapse,.attachment:hover,.correctioninfo,.notification:hover,.wide-toolbar__item,.ui-button,.color_snow_white,.menu-item:hover,.modern-message__action:hover,.ui-button-icon,.topnav__menuitem:hover,.modern-message:hover,.smsc-column-nav__button:hover,.bootstrap__input,select,.skinButton,.skinButtonBg,.wide-toolbar__item::after,.wide-toolbar__item::before,.smsc-dropzone,.modern-message--selected,.blob,.float-label__input,.input-search,.smscButton,.course-ico,.btn,checkbox {background-color:" + a["bg2_color"] + " !important; border-color:" + a["bg2_color"] + " !important;}\n.cell--spacer{border-color:" + a["bg2_color"] + " #0000 !important;}";
            bg2_color_temp = a["bg2_color"];
            document.head.appendChild(e);
        });
        await browser.storage.local.get("bg_color").then((a) => {
            let e = document.createElement("style");
            e.innerText = ".navNextInput,.navPrevInput,.color_snow_white,.fileinfowhite,.fileinfogrey,.ui-dialog-buttonpane,.ui-dialog-titlebar,.ui-dialog,.modern-message__actions,.context-menu,.notifs-toaster__toast,option,.spacer,.showLeftNav,.detailContainerBlockValue,.content_container,.contentContainer,.smsc-container,body,.smsc-contextmenu-bubble,.selectionContainer,.agenda_grid_main,.topnav__menu,.bubble,.helper--height--mega,td,.dialog-content,.side-panel__panel,.wide-toolbar,.wide-toolbar__item--selected,.topnav__menu-arrow,.msgDetail--empty,.messageframe,.toolbar,.folders,.smscleftnavcontainer,.msgContentVal,.smsc-contextmenu-arrow::before{background-color:" + a["bg_color"] + " !important;}\n.smsc-contextmenu-arrow::after,.topnav__menu-arrow::after,.ui-dialog-titlebar{border-color:" + a["bg_color"] + " #0000 !important;}";
